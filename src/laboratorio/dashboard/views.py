...
from laboratorio.models import Consumable, History, Permanent, ConsumableTI, HistoryTI, PermanentTI
...
# Novas views para Consumíveis TI
@login_required(login_url="/dashboard/login")
@allowed_users(allowed_roles=material_roles)
def consumable_ti_materials_view(request):
    return render(
        request,
        "laboratorio/dashboard/consumivel_ti/dashboard.html",
    )

@login_required(login_url="/dashboard/login")
@allowed_users(allowed_roles=material_roles)
def update_consumable_ti_view(request, pk):
    try:
        item = ConsumableTI.objects.get(id=pk)
    except ConsumableTI.DoesNotExist as err:
        raise Http404(err) from err

    context = {"item": item}
    return render(
        request,
        "laboratorio/dashboard/consumivel_ti/editar.html",
        context,
    )

@login_required(login_url="/dashboard/login")
@allowed_users(allowed_roles=material_roles)
def materials_consumable_ti_list_view(request):
    return render(
        request,
        "laboratorio/dashboard/consumivel_ti/relacao.html",
    )

# Novas views para Permanentes TI
@login_required(login_url="/dashboard/login")
@allowed_users(allowed_roles=material_roles)
def permanent_ti_materials_view(request):
    return render(
        request,
        "laboratorio/dashboard/permanente_ti/dashboard.html",
    )

@login_required(login_url="/dashboard/login")
@allowed_users(allowed_roles=material_roles)
def update_permanent_ti_view(request, pk):
    try:
        item = PermanentTI.objects.get(id=pk)
    except PermanentTI.DoesNotExist as err:
        raise Http404(err) from err

    context = {"item": item}
    return render(
        request,
        "laboratorio/dashboard/permanente_ti/editar.html",
        context,
    )

@login_required(login_url="/dashboard/login")
@allowed_users(allowed_roles=material_roles)
def materials_permanent_ti_list_view(request):
    return render(
        request,
        "laboratorio/dashboard/permanente_ti/relacao.html",
    )

# Funções auxiliares e relatórios (mantendo as mesmas)
...
def reports_ti_view(request):
    ...
    return render(
        request,
        "laboratorio/dashboard/consumivel_ti/relatorios.html",
        context,
    )
    
@login_required(login_url="/dashboard/login")
@allowed_users(allowed_roles=material_roles)
def report_ti_view(request, year):
    workbook = Workbook()

    ws = workbook.active
    ws.freeze_panes = ws["B4"]

    title = f"Relatório de entradas e saídas de TI {year}"
    filename = f"relatorio_de_entradas_e_saídas_ti_{year}.xlsx"
    operations = ["Entrada", "Saída"]
    columns = [
        "Janeiro",
        "Fevereiro",
