...
$("#deletePeriod").click(function (e) {
    $.ajax({
        type: "DELETE",
        url: `/api/laboratory/ti/consumables/${id}/`,
...
function atualizarQuantidade(serialized_data) {
    $.ajax({
        type: "POST",
        url: `/api/laboratory/ti/consumables/${id}/add/`,
        dataType: "json",
...
function fillMaterial() {
    $.get(`/api/laboratory/ti/consumables/${id}/`, (response) => {
        fillAttributes(response);
        $.get(`/api/laboratory/ti/consumables/${id}/history/`, (history) => {
            $("#history").html("");
            history.forEach((entry) => {
                ...
                let unit = `${response.measure_unit}${Math.abs(entry.diff) != 1 ? "s" : ""
                    }`;
