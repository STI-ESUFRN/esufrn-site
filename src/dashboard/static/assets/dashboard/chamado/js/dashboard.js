	...
	$.get("/api/admin/chamados/?status=P", function (response) {
		...
		if (localData != results) {
			...
			$.each(data, (index, reserve) => {
				...
				let e1 = $('<td />', { class: (((now - criado_ha_date) > 1200000) ? "font-weight-bold text-danger" : "font-weight-bold"), });
				let e2 = $('<time />', { text: criado_ha, class: "timeago", dateTime: criado_ha });
				e1.append(e2);
...
function fillCall(id) {
	...
	$("#detalhes").hide();
	$.get(`/api/admin/chamados/${idSelecionado}/`, function (response) {
...
setInterval(() => {
	refreshData();
	$("td:last-child:not(.text-danger).font-weight-bold").each((index, element) => {
		var now = new Date();
