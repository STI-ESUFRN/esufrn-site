...
$("#enviar-reserva").click(function (e) {
    ...
    error += validate(
        form_email,
        form_email.val() && isEmail(form_email.val())
    );

    form_phone = $("#form-reserva [name=phone]");
    error += validate(
        form_phone,
        form_phone.val() && isPhone(form_phone.val())
    );
    ...
    if (!error) {
        ...
        serialized_data["declare"] = $("#form-reserva [name=declare]").is(
            ":checked"
        );
        serialized_data["status"] = $("#form-reserva [name=status]").is(
            ":checked"
        )
            ? "A"
            : "E";
        ...
        $.ajax({
            ...
            success: function (response) {
                showMessage("Reserva cadastrada com sucesso.", "alert-success");
                formReset();
            },
