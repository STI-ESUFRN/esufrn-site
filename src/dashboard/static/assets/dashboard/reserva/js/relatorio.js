...
function update() {
    ...
    $(".planilha-data").text(
        `Reservas do mês de ${mesFullNome[mesAtual - 1]} - ${anoAtual}`
    );
    ...
    $(".reservation-calendar").each(function () {
        ...
        $.ajax({
            type: "GET",
            url: `/api/calendar/?date__year=${anoAtual}&date__month=${mesAtual}&classroom=${id}&status=A`,
            dataType: "json",
            ...
            error: function (response) {
                $(self).html(
                    `<h5 class="text-danger"><b>Não foi possível obter informações sobre o calendário (${response.status})</b></h5>`
                );
            },
        });
    });
