...
from reserva.serializers import (PeriodReserveBasicSerializador,
                                 PeriodReserveDayPublicSerializador,
                                 PeriodReserveDaySerializador,
                                 ReservePublicSerializador,
                                 ReserveSerializador)
...
class calendarioView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        periodreserves = PeriodReserveDay.objects.filter(
            date__year=year, date__month=month).exclude(active=False)
        ...
        try:
            data_reserves = []
            if reserves:
                serializador = ReservePublicSerializador(
                    reserves, many=True)
                data_reserves += serializador.data

            if periodreserves:
                serializador = PeriodReserveDayPublicSerializador(
            ...
            return JsonResponse({"message": "".format(str(e)), "status": "error"}, safe=False)
...
class reservasAdminView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if user.is_superuser:
            ...
            classrooms = UserClassroom.objects.filter(
                user=request.user).values_list("classroom", flat=True)

        reserves = Reserve.objects.filter(classroom__in=classrooms).exclude(status='E').order_by(
            'created_at' if orde == 'dec' else '-created_at')

        if request.GET.get("status"):
            reserves = reserves.filter(status=request.GET.get("status"))

        serializador = ReserveSerializador(
            paginate(reserves, page, npp), many=True)
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            try:
                return JsonResponse(
                    {"message": "{}".format(next(iter(e))), "status": "error"}, safe=False, status=200
                )
            except:
                return JsonResponse(
                    {"message": "{}".format(e), "status": "error"}, safe=False, status=200
                )
...
class reservasView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        serializador = ReserveSerializador(reserves, many=True)
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return JsonResponse(
                {
                    "message": "Solicitação realizada com sucesso! Espere a confirmação da reserva por email.",
                    "status": "success",
                },
                safe=False,
            )
...
class reservaAdminView(View):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            serializador = ReserveSerializador(result, many=False)
            return JsonResponse(serializador.data, safe=False)
        except:
            raise Http404()
    ...
    def put(self, request, *args, **kwargs):
        ...
        try:
            ...
            return JsonResponse({'message': 'Arquivo atualizado com sucesso!', 'status': 'success'}, safe=False)

        except Exception as e:
            try:
                return JsonResponse(
                    {"message": "{}".format(next(iter(e))), "status": "error"}, safe=False)
            except:
                return JsonResponse(
...
class periodsAdminView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            if request.GET.get("shift"):
                result = result.filter(
                    shift__contains=request.GET.get("shift"))

            if request.GET.get("status"):
                result = result.filter(
                    status=request.GET.get("status"))

            serializador = PeriodReserveBasicSerializador(
            ...
            return JsonResponse({'message': str(e), 'status': 'error'}, safe=False, status=200)
...
class periodAdminView(View):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            return JsonResponse({'message': 'Arquivo não esiste', 'status': 'error'}, safe=False, status=200)
    ...
    def put(self, request, *args, **kwargs):
        ...
        try:
            ...
            if dic.get("classroom"):
                # Chaves estrangeiras aparentemente não são atualizadas por dicionário
                dic['classroom'] = Classroom.objects.get(
                    id=int(dic.get("classroom")))
                if dic['classroom'] != reserve.classroom:
                    reserve.classroom = dic['classroom']
                    update_days = True
                del dic['classroom']

            # Remove a carga horária caso não seja especificada, pois este é um campo inteiro
...
class periodDaysAdminView(View):
    ...
    def get(self, request, *args, **kwargs):
        days = PeriodReserveDay.objects.filter(
            period=kwargs['period_pk']).order_by("id")
        serializador = PeriodReserveDaySerializador(days, many=True)
...
class periodDayAdminView(View):
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            serializador = PeriodReserveDaySerializador(reserve, many=False)

            return JsonResponse(serializador.data, safe=False)

        except:
            return JsonResponse({"message": "Não existe", "status": "error"}, safe=False)
